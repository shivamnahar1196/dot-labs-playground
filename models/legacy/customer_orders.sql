...
from {{ source('dbt_snahar', 'jaffle_shop_orders') }} as orders

join (
      select 
        first_name || ' ' || last_name as name, 
        * 
      from {{ source('dbt_snahar', 'jaffle_shop_customers') }}
) customers
...
join (
    ...
    from (
      select 
        ...
        *
      from {{ source('dbt_snahar', 'jaffle_shop_orders') }}
    ) a
    ...
    left outer join {{ source('dbt_snahar', 'stripe_payments') }} c
    on a.id = c.orderid
...
left outer join {{ source('dbt_snahar', 'stripe_payments') }} payments
